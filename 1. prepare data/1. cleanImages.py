...
class Application(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.v = tk.IntVar()
        self.radioButton1 = tk.Radiobutton(master=self.frameMain, text= "Apply New Metadata", variable=self.v, value=1)
    ...
    def startProcess(self):
        ...
        if multipleTimes == "":
            multipleTimes = 1
        itr = 0
        self.process(vars, folder, varR, multipleTimes, itr)
        if vars[10] == 1:
            self.shuffleImages(vars)
        self.labelProgess["text"] = "Done!"

    def shuffleImages(self, vars):
    ...
    def process(self, vars, folder, varR, multipleTimes, itr):
        for i in range(int(multipleTimes)):
            itr = itr + 1
            self.createNewFolders(folder, itr)
            for root, dirs, files in os.walk(folder, topdown = False):
                self.processEveryFolder(vars, varR, itr, root, files)

    def processEveryFolder(self, vars, varR, itr, root, files):
        for file in files:
            fileName = os.path.join(root, file)
            fileext = os.path.splitext(fileName)[1]
            if fileext == ".jpeg" or fileext == ".png" or fileext == ".jpg":
                self.processImage(vars, varR, itr, root, file)

    def checkFunction1(self, img, fileName):
        if vars[0] == 1:
            h, w, _ = img.shape
            if w >= h:
                os.remove(fileName)
    ...
    def checkFunction2(self, img, fileName):
        if vars[1] == 1:
    ...
    def checkFunction3(self, img, fileName):
        if vars[2] == 1:
    ...
    def checkFunction4(self, img, fileName):
        if vars[3] == 1:
            cascPath = "./haarcascade_frontalface_default.xml"
            faceCascade = cv2.CascadeClassifier(cascPath)
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    def checkFunction5(self, img, fileName):
        if vars[4] == 1:
            h, w, _ = img.shape
            if h > 1920 or w > 1080:
                os.remove(fileName)
    ...
    def checkFunction6(self, img, fileName):
        if vars[5] == 1:
            h, w, _ = img.shape
            if h < 654 or w < 420:
                os.remove(fileName)
    ...
    def checkFunction7(self, img, fileName):
        if vars[10] == 1:
            cascPath = "./haarcascade_frontalface_default.xml"
            faceCascade = cv2.CascadeClassifier(cascPath)
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    def function8(self, img, fileName):
        if vars[6] == 1:
            image = img
    ...
    def function9(self, img, fileName):
        if vars[8] == 1:
    ...
    def function11(self, varR, fileName):
        if varR == 1:
            pass
        elif varR == 2:
    ...
    def processImage(self, vars, varR, itr, file):
        d = os.path.dirname(fileName)
        de = os.path.split(d)[1]
        destinationDir = f"./cleaned_images/{itr}cleaned_" + de
        destination = os.path.join(destinationDir, file)
        img = cv2.imread(fileName)
        if self.checkFunction1(img, fileName):
            return
        if self.checkFunction2(img, fileName):
            return
        if self.checkFunction3(img, fileName):
            return
        if self.checkFunction4(img, fileName):
            return
        if self.checkFunction5(img, fileName):
            return
        if self.checkFunction6(img, fileName):
            return
        if self.checkFunction7(img, fileName):
            return
        shutil.copyfile(fileName,destination)
        fileName = destination
        self.function8(img, fileName)
        self.function9(img, fileName)
        self.function10(img)
        self.function11(varR, fileName)
